...
// 服务状态
const services = ref([
  { name: 'API服务', description: '充电桩后端API服务', status: 'running', statusText: '运行中', uptime: '15天3小时' },
  { name: '数据库服务', description: 'MySQL数据库服务', status: 'running', statusText: '运行中', uptime: '15天3小时' },
  { name: '缓存服务', description: 'Redis缓存服务', status: 'running', statusText: '运行中', uptime: '15天3小时' },
  { name: '消息队列', description: 'RabbitMQ消息服务', status: 'running', statusText: '运行中', uptime: '15天3小时' },
  { name: 'Web服务', description: 'Nginx Web服务器', status: 'running', statusText: '运行中', uptime: '15天3小时' },
  { name: '监控服务', description: 'Prometheus监控', status: 'warning', statusText: '警告', uptime: '2天5小时' }
])
...
const logs = ref([
  { id: 1, level: 'info', time: '15:30:25', message: '用户登录成功', source: 'AuthService' },
  { id: 2, level: 'info', time: '15:28:12', message: '充电桩#001连接成功', source: 'DeviceService' },
  { id: 3, level: 'warning', time: '15:25:43', message: 'CPU使用率超过80%', source: 'MonitorService' },
  { id: 4, level: 'info', time: '15:22:18', message: '充电记录保存成功', source: 'RecordService' },
  { id: 5, level: 'error', time: '15:20:05', message: '数据库连接超时', source: 'DatabaseService' },
  { id: 6, level: 'info', time: '15:18:32', message: '定时任务执行完成', source: 'ScheduleService' },
  { id: 7, level: 'warning', time: '15:15:47', message: '磁盘空间不足20%', source: 'SystemService' },
  { id: 8, level: 'info', time: '15:12:21', message: '缓存更新成功', source: 'CacheService' }
])
...
const refreshStatus = () => {
  ...
  setTimeout(() => {
    ...
    metrics.value.memory = Math.floor(Math.random() * 20) + 60
    metrics.value.disk = Math.floor(Math.random() * 20) + 40
    metrics.value.network = Math.floor(Math.random() * 40) + 20
...
onMounted(() => {
  ...
  autoRefreshTimer = setInterval(() => {
    ...
    metrics.value.network = Math.min(200, Math.max(10, metrics.value.network + (Math.random() - 0.5) * 20))
  }, 30000)
})
