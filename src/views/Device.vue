    ...
    <div class="stats-grid">
      ...
      <div class="stat-card">
        ...
        <div class="stat-info">
          ...
          <div class="stat-label">今日充电量</div>
        </div>
      </div>
    </div>
...
const chargingCount = computed(() => 
  devices.value.filter(d => d.status === 'charging').length
)

const totalPower = computed(() => 
  devices.value.reduce((sum, d) => sum + d.power, 0)
)

const totalEnergy = computed(() => 
  devices.value.reduce((sum, d) => sum + d.chargedEnergy, 0)
)
